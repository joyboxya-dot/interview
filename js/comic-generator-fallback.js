/**
 * 서버 없을 때 topicComic data URI 생성 (speech-server/comic-generator.js 와 동일 로직)
 */
(function (global) {
    const W = 200;
    const H = 150;
    ...
    function pickStarLabels(topic) {
        let ko = (topic.sentences || [])
            .map(function (s) {
                return (Array.isArray(s) ? s[0] : s.ko) || '';
            })
            .filter(Boolean);
        ...
        if (!ko.length) {
            ...
            return [t, t, t, t].map(function (x) {
                return shortenKo(x, 14);
            });
        }
        const n = ko.length;
        if (n === 1) {
            return [ko[0], ko[0], ko[0], ko[0]].map(function (x) {
                return shortenKo(x, 14);
            });
        }
        if (n === 2) {
            return [ko[0], ko[1], ko[1], ko[1]].map(function (x) {
                return shortenKo(x, 14);
            });
        }
        if (n === 3) {
            return [ko[0], ko[1], ko[2], ko[2]].map(function (x) {
                return shortenKo(x, 14);
            });
        }
        const i1 = Math.max(1, Math.floor(n / 3));
        const i2 = Math.max(i1 + 1, Math.floor((2 * n) / 3));
        return [ko[0], ko[i1], ko[i2], ko[n - 1]].map(function (x) {
            return shortenKo(x, 14);
        });
    }
    ...
    function detectVisual(label) {
        ...
        if (/장애|개장|배치|실시간/.test(t)) return 'alert';
    ...
    function drawVisual(kind, stepIndex) {
        switch (kind) {
            ...
            case 'success':
                return (
                    '<circle cx="85" cy="58" r="28" fill="#ECFDF5" stroke="#059669" stroke-width="3"/>' +
                    '<text x="85" y="66" text-anchor="middle" font-size="22" fill="#059669">✓</text>' +
                    badge(118, 42, 'OK', '#D1FAE5', '#047857')
                );
            case 'join':
                return (
                    person(50, 42) +
                    '<line x1="88" y1="62" x2="130" y2="62" stroke="#6366F1" stroke-width="4"/>' +
                    person(145, 42, '#059669')
                );
            case 'check':
            ...
            default:
                ...
                if (stepIndex === 1) {
                    return badge(55, 42, '?', '#FEE2E2', '#B91C1C') + monitor(40, 58, 120, 42, '…');
                }
                if (stepIndex === 2) return person(70, 42) + monitor(110, 38, 70, 60, 'act');
                return (
                    '<circle cx="85" cy="58" r="26" fill="#ECFDF5" stroke="#059669" stroke-width="3"/>' +
                    '<text x="85" y="66" text-anchor="middle" font-size="20" fill="#059669">✓</text>'
                );
        }
    }

    function buildPanelSvg(stepTag, label, stepIndex) {
        return (
            ...
            '<text x="36" y="23" text-anchor="middle" font-family="system-ui,sans-serif" font-size="11" font-weight="800" fill="#4338CA">' +
            esc(stepTag) +
            '</text>' +
            '<rect x="0" y="108" width="' +
            W +
            '" height="42" fill="#FFFFFF"/>' +
            '<line x1="0" y1="108" x2="' +
            W +
            '" y2="108" stroke="#E2E8F0" stroke-width="1"/>' +
            drawVisual(detectVisual(label), stepIndex) +
            '<text x="' +
            W / 2 +
            '" y="132" text-anchor="middle" font-family="system-ui,-apple-system,sans-serif" font-size="12" font-weight="700" fill="#1E293B">' +
            esc(label) +
            '</text></svg>'
        );
    }
    ...
    function generateTopicComic(topic) {
        ...
        const panels = labels.map(function (label, i) {
            return svgToDataUri(buildPanelSvg(STEP_TAGS[i], label, i));
        });
        return {
            topicComic: {
                captionKo: labels.join(' → '),
                panels: panels,
            },
        };
    }
